...
- Headless Codex ACP smoke (local, not CI): `npm run smoke:codex-acp` spawns `codex_spawn_argv` (`codex-acp` or `npx -y @agentclientprotocol/codex-acp`), stdio `initialize`, asserts protocolVersion 1 + `loadSession` + authMethods (`api-key`, `chat-gpt`). Covers Mac-common `codex`-only PATH without GUI.
