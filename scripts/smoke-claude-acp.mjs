#!/usr/bin/env node
/**
 * Headless Mac/Linux smoke: spawn Claude ACP the same way ClaudeBackend does
 * (`claude-agent-acp` on PATH, else `npx -y @agentclientprotocol/claude-agent-acp`), then
 ...
 *   claude_is_detectable / claude_spawn_argv
 *
 * Requires a local Claude Code login (Pro/Max login or ANTHROPIC_API_KEY). Not CI.
 * Local: npm run smoke:claude-acp
 *
 * Env:
 *   CLAUDE_ACP_SMOKE_TIMEOUT_MS  per-request timeout (default 120000)
 *   CLAUDE_ACP_SMOKE_SKIP_PROMPT=1  skip session/prompt (still does new+load)
 */
...
const TIMEOUT_MS = Number(process.env.CLAUDE_ACP_SMOKE_TIMEOUT_MS || 120_000);
const SKIP_PROMPT = process.env.CLAUDE_ACP_SMOKE_SKIP_PROMPT === "1";
const TAG = "smoke-claude-acp";
...
/** Same rule as `claude_is_detectable` / `claude_spawn_argv` in agent_backend.rs */
function resolveClaudeSpawn() {
  const hasClaudeAgentAcp = onPath("claude-agent-acp");
  const hasClaude = onPath("claude");
  const detectable = hasClaudeAgentAcp || hasClaude;
  const argv = hasClaudeAgentAcp
    ? ["claude-agent-acp"]
    : ["npx", "-y", "@agentclientprotocol/claude-agent-acp"];
  return { hasClaudeAgentAcp, hasClaude, detectable, argv };
}
...
async function main() {
  const resolved = resolveClaudeSpawn();
  console.log(
    `detect: claude-agent-acp=${resolved.hasClaudeAgentAcp} claude=${resolved.hasClaude} → spawn ${JSON.stringify(resolved.argv)}`,
  );

  if (!resolved.detectable) {
    fail(
      "neither `claude-agent-acp` nor `claude` on PATH (ClaudeBackend.detect would be false)",
    );
  }

  if (resolved.hasClaudeAgentAcp) {
    if (JSON.stringify(resolved.argv) !== JSON.stringify(["claude-agent-acp"])) {
      fail("claude_spawn_argv(true) mismatch");
    }
  } else {
    const expected = ["npx", "-y", "@agentclientprotocol/claude-agent-acp"];
    if (JSON.stringify(resolved.argv) !== JSON.stringify(expected)) {
      fail(`claude_spawn_argv(false) mismatch: got ${JSON.stringify(resolved.argv)}`);
    }
    ok("Mac-common path: only `claude` present → npx fallback argv");
  }

  const cwd = mkdtempSync(join(tmpdir(), "acp-desktop-claude-smoke-"));
  ...
  try {
    ...
    try {
      ...
      fail(
        `session/new failed (Claude auth?): ${err.message || err}\n` +
          `hint: log in via \`claude\` (Pro/Max) or set ANTHROPIC_API_KEY\n` +
          `stderr:\n${agent.getStderr()}`,
      );
    }
    ...
    if (!SKIP_PROMPT) {
      ...
      if (agent.permissionCount > 0) {
        ...
        ok("no session/request_permission this turn (Claude may not Ask for a plain reply)");
      }
      ...
      ok("skipped session/prompt (CLAUDE_ACP_SMOKE_SKIP_PROMPT=1)");
    }
